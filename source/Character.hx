...
class Character extends FlxSprite
{
	...
	public var danceLeftGuy:Bool = false;
		...
		switch (curCharacter)
		{
			...
			default:
				...
				if(Assets.exists(Paths.image("characters/" + curCharacter + "_assets")))
				{
					...
					if(Assets.exists(Paths.image("characters/" + curCharacter + "_offsets").replace(".png", ".txt")))
					{
						var lines = CoolUtil.coolTextFile(Paths.image("characters/" + curCharacter + "_offsets").replace(".png", ".txt"));
						trace(lines);

						for(line in lines)
						{
							addOffset(line.split(" ")[0], Std.parseInt(line.split(" ")[1]), Std.parseInt(line.split(" ")[2]));
							animation.addByPrefix(line.split(" ")[0], line.split(" ")[0], 24, false);
						}
